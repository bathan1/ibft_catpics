...
bool Node::verify_signature(const std::string &hash, const std::string &signature, int sender_id) {
    EVP_PKEY *sender_pubkey = public_keys[sender_id];
...
void Node::receive(const Message &msg) {
    switch (msg.type) {
        case PREPARE:
            ...
            if (verify_signature(hash, msg.signature, msg.sender)) {
